# Disclaimer

## Medical Disclaimer
This project is for **research and educational purposes only**. It does not constitute medical advice, diagnosis, or treatment recommendation. Do not make any medical decisions based on the content of this repository.

## Not Clinical Research
The author (Igor Mihaljko) is not a licensed medical professional, genetic counselor, or clinical researcher. The genomic data presented are from consumer genetic testing services (Genetic Genie / GenVue Discovery) and have not been confirmed by clinical-grade testing.

## Computational Predictions
AlphaFold structural predictions are computational models and may not accurately reflect biological reality. All structural comparisons require experimental validation before any conclusions can be drawn about therapeutic viability.

## Disease Associations
The MTHFR-disease associations described in this project represent published correlations from peer-reviewed literature. Correlation does not equal causation. The significance and clinical relevance of common MTHFR variants remains an area of active scientific debate. The CDC states that MTHFR variants are generally not a reason to change medical treatment.

## Therapeutic Concepts
All gene therapy concepts described are speculative and would require extensive preclinical testing, safety validation, and clinical trials before any human application. The regulatory, ethical, and safety considerations for gene editing therapies are substantial.

## AlphaFold Server Terms
AlphaFold Server outputs are for non-commercial use only, subject to [AlphaFold Server Terms of Service](https://alphafoldserver.com/terms). Outputs cannot be used for commercial drug discovery, docking/screening tools, or training ML models.

## No Warranty
All information is provided "as-is" without any warranty. The authors are not responsible for any consequences arising from the use or misuse of this information.
